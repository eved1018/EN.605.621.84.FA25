...
def fastest_tour_wrapper_permute(L, TRAVEL_TIME):
    best_tour = []
    best_time = inf
    best_steps = 0
    best_start = None
    for start_light in L:
        L_minus = list_minus(L, start_light)
        curr_tour, curr_time = bf_permute(start_light, L_minus, TRAVEL_TIME)
        if curr_time < best_time:
            best_tour = curr_tour
            best_time = curr_time
            best_start =  start_light
...
def fastest_tour_bf(start_light: str, L: list[str], TRAVEL_TIME: dict[tuple[str, str], float]) -> tuple[list[str], float, int]:
    ...
    if len(L) == 0:
        raise ValueError(f"List of lighthouses must contain items. Instead {L=}")

    # BASE CASE - if one lighthouse left return the distance btw it and start_light
    if len(L) == 1:
        # Add code to calculate the base case here
        next_light = L[0]
        best_tour = [start_light, next_light]
        best_time = get_travel_time(start_light, next_light, TRAVEL_TIME)
        spaced = " " 
        print(" " , 1, spaced, start_light, best_tour, best_time)
    ...
    for second_light in L:
        ...
        # remove possible next lighthouse
        L_minus = list_minus(L, second_light)
        curr_tour, curr_time, steps = fastest_tour_bf(second_light, L_minus, TRAVEL_TIME)
        all_steps += steps
    ...
    spaced = " " * (all_steps)
    print(" ", all_steps, spaced, start_light, best_tour, best_time)
    best_tour = [start_light] + best_tour
    return best_tour, best_time, all_steps


def bf_permute(s, L, TT):
    from itertools import permutations
    best_tour = []
    best_time = inf
    L_prime = list_minus(L, s)
    perms = list(permutations(L_prime))
    for p in perms:
        t = get_travel_time(s, p[0], TT)
        for x, y in zip(p, p[1:]):
            t += get_travel_time(x, y, TT)
        if t < best_time:
            best_time = t
            best_tour = list(p)

    tour = [s] + best_tour
    return tour, best_time
...
def test1():
    ...
    print(L)
    best_tour, best_time, steps, best_start = fastest_tour_wrapper(L, TRAVEL_TIME)
    print(best_tour, best_time, best_start)
    answer = ["A", "B", "C", "D", "E"]
    assert best_tour == answer or best_tour[::-1] == answer, f"{best_tour} != {answer}"
    assert best_time == 28.089873889890804
    best_tour, best_time, steps, best_start = fastest_tour_wrapper_permute(L, TRAVEL_TIME)
    print(best_tour, best_time, best_start)
    assert best_tour == answer or best_tour[::-1] == answer, f"{best_tour} != {answer}"
    assert best_time == 28.089873889890804
...
def test2():
    ...
    L = list(set([item for k in TRAVEL_TIME.keys() for item in k]))
    print(L)
    best_tour, best_time, steps = fastest_tour_wrapper(L, TRAVEL_TIME)
    answer = ["A", "B", "C", "D", "E"]
    assert best_tour == answer or best_tour[::-1] == answer, f"{best_tour} != {answer}"
